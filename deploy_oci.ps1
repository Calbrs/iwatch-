...
ssh -i $key -o BatchMode=yes -o StrictHostKeyChecking=no $box "cd ~/iwatch && git pull --ff-only && sudo systemctl restart iwatch-tracker"

Write-Host "== 3/4 wait for service =="
Start-Sleep -Seconds 5

Write-Host "== 4/4 verify deployed commit =="
ssh -i $key -o BatchMode=yes -o StrictHostKeyChecking=no $box "cd ~/iwatch && git log --oneline -1 && sudo systemctl is-active iwatch-tracker"
Write-Host "DONE"
